...
3. Write the working draft to **this session's own draft file**, `~/.claude/voice/sessions/$CLAUDE_CODE_SESSION_ID/draft.md` — a per-session path so two agents drafting at once never share a file. Ensure the dir exists (`mkdir -p ~/.claude/voice/sessions/$CLAUDE_CODE_SESSION_ID`). The Write tool needs a literal path, so resolve it first: `echo ~/.claude/voice/sessions/$CLAUDE_CODE_SESSION_ID/draft.md`. Draft in this thread, with full conversation context, under the one rule plus tells plus craft plus voice plus learnings. Learnings are recent, specific overrides: where one conflicts with the profile or a craft default, the learning wins. Keep the draft file equal to the text you intend to deliver.
4. Gate it. Pipe that exact file: `python3 ~/.claude/skills/voice/check.py < ~/.claude/voice/sessions/$CLAUDE_CODE_SESSION_ID/draft.md`. A FAIL never ships. Fix the draft file and re-run, up to THREE attempts. The judge is a LINTER, not a critic: it may only flag text that violates a named rule in `tells.md`, a profile law, or a learning, must cite that rule verbatim, and check.py drops any flag citing a rule that doesn't exist. Re-runs in the same session only lint the lines you changed, so the loop converges. If a flag is a real word of yours the gate keeps catching, add it to `~/.claude/voice/allowlist.txt` instead of mangling the sentence.
5. If it still FAILs after three attempts, STOP. Do not deliver it and do not override the judge yourself. Surface the exact remaining flags to the user, say which you think are false positives and why, and ask whether to override or keep iterating. Only on explicit user approval, record the override by writing the draft's normalized sha256 into the shared overrides set: `python3 -c "import hashlib,os,pathlib;d=pathlib.Path.home()/'.claude/voice';t=(d/'sessions'/os.environ['CLAUDE_CODE_SESSION_ID']/'draft.md').read_text().replace(chr(13)+chr(10),chr(10)).replace(chr(13),chr(10)).strip();o=d/'overrides';o.mkdir(exist_ok=True);(o/hashlib.sha256(t.encode()).hexdigest()).write_text('override')"`. Never ship a FAILing draft silently.
6. For anything substantial or outward-facing (more than a few lines, or going to someone outside), spawn the `critic` agent with the draft, `about-me.md`, and `tells.md`. Apply its fixes, update the draft file, re-run the gate.
7. Output only the final text, and it must equal the draft file. Don't mention the rules or the scan unless asked.

**There is no automated backstop.** This skill used to be enforced by three hooks — a `UserPromptSubmit` arm hook, a `Stop` gate, and a `PreToolUse` outbound guard on `create_draft`/`slack_send_message`. All three were removed deliberately: they fired on any prompt that merely mentioned writing or a customer, which was wrong far more often than it was right. `/voice` is now invoke-only.

The consequence: "a FAIL never ships" is a rule you keep, not one anything enforces. Nothing blocks a turn ending on an unverified draft, and nothing intercepts a Gmail or Slack call whose body never saw the gate. Run the route in full or don't claim you ran it.
...
## Deterministic check
Gate the exact file you will deliver, so the PASS receipt the Stop hook and the outbound guard trust matches it:
```bash
python3 ~/.claude/skills/voice/check.py < ~/.claude/voice/sessions/$CLAUDE_CODE_SESSION_ID/draft.md
```
A draft that prints FAIL does not ship until it prints PASS (see the Write route for the 3-try-then-escalate rule, and never override the judge on your own). The LLM judge is a linter against an explicit rulebook: `tells.md` (the floor), `about-me.md` (the author's laws, which make patterns like flat imperatives and colon-led labels legal), and `learnings/` (applied literally, never generalized). Every flag must quote its rule verbatim, tagged `floor:` / `law:` / `learning:<file>:`; check.py mechanically drops flags citing rules that don't exist, and prints them as `# dropped` notes. Within a session, re-running an edited draft lints only the changed lines and an identical draft reuses the cached verdict, so the fix loop converges instead of re-rolling new objections. Backend: codex at low reasoning effort (~11s per lint), claude as fallback (its CLI carries ~100s of startup overhead regardless of model). On a full PASS it writes a content-keyed receipt — a file named by the body's normalized sha256 — into the shared set `~/.claude/voice/receipts/`. Because receipts are a set keyed by content, concurrent voice drafts never clobber each other's PASS. Nothing reads the receipts automatically any more — they are an audit trail of what the judge approved, not a gate. Add `--no-llm` to skip the judge (no receipt is written then). Words and phrases the user legitimately writes live in `~/.claude/voice/allowlist.txt` (one per line, `#` for comments); the static gate and the judge both skip them, so a real person's vocabulary and signature phrases survive the floor. `tells.md` and `craft.md` still apply where the script cannot see.
